...
//require_once DOL_DOCUMENT_ROOT .'/core/modules/facture/mod_facture_wsfe.php';
global $db,$conf;
...
//~ include_once DOL_DOCUMENT_ROOT . '/wsfephp/wsfe_doli.php';
		    //~ $resultqq = wsfe_doli($db, $object, $langs, $conf, $user, $idwarehouse);
		    //~ $numfinal;
		$obj_facturation->numInvoice($num);
		//~ $obj_facturation->numInvoice($numfinal);
	...
	case 'valide_facture':
		...
		else
		{
			...
			if ($resultcreate > 0)
			{
				...
				if (! empty($conf->global->CASHDESK_NO_DECREASE_STOCK)) $warehouseidtodecrease=0;	// If a particular stock is defined, we disable choice
				//~ include_once DOL_DOCUMENT_ROOT . '/wsfephp/wsfe_doli.php';
				//~ $resultvalid = wsfe_doli($db, $object, $langs, $conf, $user, $idwarehouse);
				$resultvalid=$invoice->validate($user, $obj_facturation->numInvoice(), 0);
